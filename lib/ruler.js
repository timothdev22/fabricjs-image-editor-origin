...
(function () {
  ...
  var ruler = function() {
    ...
    // Redimensionar as réguas baseado no tamanho do canvas
    this.resizeRulers = function() {
      if (!this.canvas) return; // Verificar se o canvas existe
      
      this.logCanvasDimensions('resizeRulers');
      ...
      if (!this.horizontalRuler || !this.verticalRuler) return;
      
      // Ajustar o tamanho dos canvas das réguas
      ...
      console.log('Dimensões das réguas - H:', this.horizontalRuler.width, 'x', this.horizontalRuler.height,
                  '- V:', this.verticalRuler.width, 'x', this.verticalRuler.height);
    ...
    this.drawRulers = function() {
      ...
      try {
        ...
        // Calcular ponto de início baseado no viewport
    ...
    this.addGuide = function(isHorizontal, position) {
      ...
      try {
        ...
        console.log(`Adicionando guia ${isHorizontal ? 'horizontal' : 'vertical'} na posição`, position);
        ...
        console.log(`Posição ajustada no canvas: ${canvasPosition}`);
        ...
        console.log('Guia criada com sucesso:', guide);
        return guide;
      } catch (error) {
    ...
    this.setupRulerInteractions = function() {
      ...
      $(`${this.containerSelector} .horizontal-ruler`).mousedown(function(e) {
        ...
        console.log('Clique na régua horizontal');
        ...
        console.log('Posição Y do clique:', y);
        console.log('Posição da régua horizontal:', rulerRect.top, rulerRect.bottom);
        
        const guide = _self.addGuide(true, y);
        if (guide) {
          console.log('Guia horizontal adicionada em Y:', y);
        } else {
          console.error('Falha ao adicionar guia horizontal');
        }
      });
      ...
      $(`${this.containerSelector} .vertical-ruler`).mousedown(function(e) {
        ...
        console.log('Clique na régua vertical');
        // Obter a posição X exata do clique em relação à janela
        const x = e.clientX;
        
        // Obter mais informações para debug
        const rulerRect = this.getBoundingClientRect();
        console.log('Posição X do clique:', x);
        console.log('Posição da régua vertical:', rulerRect.left, rulerRect.right);
        
        const guide = _self.addGuide(false, x);
        if (guide) {
          console.log('Guia vertical adicionada em X:', x);
        } else {
          console.error('Falha ao adicionar guia vertical');
        }
      });
      ...
      if (this.canvas) {
          ...
          if (e.target.metadata && e.target.metadata.isGuide) {
            try {
              ...
              if (e.target.metadata.isHorizontal) {
                ...
                console.log('Movendo guia horizontal:', e.target.top);
                ...
                // É uma guia vertical - mover apenas horizontalmente
                console.log('Movendo guia vertical:', e.target.left);
        ...
        this.canvas.on('object:modified', function(e) {
          ...
          if (target && target.metadata && target.metadata.isGuide) {
            console.log('Guia modificada, atualizando aparência');
      ...
      this.canvas.on('selection:created', function(e) {
        if (e.selected && e.selected.length === 1 && e.selected[0].metadata && e.selected[0].metadata.isGuide) {
          ...
          guide.active = true;
          _self.canvas.renderAll();
          console.log('Guia selecionada:', guide.metadata.isHorizontal ? 'horizontal' : 'vertical');
        }
      });
      ...
      this.canvas.on('selection:updated', function(e) {
        ...
        if (e.selected && e.selected.length === 1 && e.selected[0].metadata && e.selected[0].metadata.isGuide) {
          ...
          guide.active = true;
          _self.canvas.renderAll();
          console.log('Seleção atualizada para guia:', guide.metadata.isHorizontal ? 'horizontal' : 'vertical');
        }
      });
      ...
      this.canvas.on('selection:cleared', function() {
        ...
        if(guides.length > 0){
          ...
          _self.canvas.renderAll();
          console.log('Seleção removida, restaurando aparência das guias');
        }
      });
    ...
    this.updateGuidesAfterCanvasChange = function() {
      ...
      console.log('Atualizando guias - Dimensões do canvas:', canvasWidth, 'x', canvasHeight);
      ...
      guides.forEach(guide => {
        try {
          if (guide.metadata.isHorizontal) {
            ...
            console.log(`Atualizada guia horizontal em Y: ${y}`);
          } else {
            ...
            console.log(`Atualizada guia vertical em X: ${x}`);
          }
        } catch (error) {
      ...
      this.canvas.renderAll();
      console.log(`Atualizadas ${guides.length} guias após mudança no canvas`);
    ...
    this.forceGuidesMovable = function() {
      ...
      console.log('Forçando guias movíveis...');
      ...
      this.canvas.renderAll();
      console.log(`${guides.length} guias configuradas para movimento forçado`);
    };
  };
  ...
  if (window.app) {
    window.app.setupRulerAfterCanvasReady = function() {
      try {
        ...
        console.log('Réguas configuradas com sucesso após inicialização do canvas');
      } catch (error) {
