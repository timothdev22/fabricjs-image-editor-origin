...
(function () {
  ...
  var defaultButtons = [{
    ...
    name: 'images',
    title: 'Images',
    icon: `<svg  width="16" height="16" fill="currentColor" class="bi bi-image" viewBox="0 0 16 16">
  ...
  var toolbar = function () {
    ...
    try {
      ...
      // zoom
      (() => {
        let currentZoomLevel = 1; // Define um valor padrão de zoom inicial (100%)
        $(`${this.containerSelector} #footerbar`).append(
          `<div class="zoom-level-container"></div>`
        );
        $(`${this.containerSelector} .zoom-level-container`).append(`
          <label for="input-zoom-level">Zoom</label>
          <input type="range" id="input-zoom-level" min="0.05" max="3" step="0.05" value="${currentZoomLevel}" oninput="updateZoomValue(this.value)">
        ...
        // Inicializar o zoom como "fit" se necessário
