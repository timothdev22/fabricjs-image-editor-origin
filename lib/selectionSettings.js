  ...
  var selectionSettings = function () {
    ...
    (() => {
          ...
          <div class="animate">
            <div class="input-container">
              ...
              <select id="animate-type">
                ...
                <option value="defaultEasing">Default Easing</option>
      ...
      $(`${this.containerSelector} .toolpanel#select-panel .animate-section #animate-type`).change(function () {
        let animate = $(this).val();

        function float(obj, distance = 20, canvas) {
        ...
        const animations = {
          // slideIn: (obj) => slideIn(obj, 'left', _self.canvas),
          // fade: (obj) => fade(obj, 1, _self.canvas),
          // zoom: (obj) => zoom(obj, 2, _self.canvas),
          // spin: (obj) => spin(obj, 360, _self.canvas),
          // swing: (obj) => swing(obj, _self.canvas),
          float: (obj) => float(obj, 30, _self.canvas),
          // typewriter: (obj) => typewriter(obj, _self.canvas),
      };

      const animation = 'float';//e.target.value;
    const obj = _self.activeSelection;
    if (obj && animations[animation]) {
      _self.animations[animation](obj);
    }

        // if(animate === "none") {
        //   _self.activeSelection.animation = null;
        // } else {
        //   _self.activeSelection.animation = {
        //     property: "left",
        //     to: _self.activeSelection.left ?? 0,
        //     duration: 800,
        //     easing: animate,
        //   };   
        //   _self.animateObject(_self.activeSelection);
