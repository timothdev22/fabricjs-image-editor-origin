  ...
  var selectionSettings = function () {
    ...
    (() => {
      ...
      const updateGradientFill = () => {
        console.log(_self.activeSelection)
    ...
    this.setSelectionValues = () => {
        ...
        if(count == 1) {
          let activeObject = _self.activeSelection[0];         

        
          if(activeObject) {

            const borderWidth = activeObject.strokeWidth;
            const borderColor = activeObject.stroke;
            const borderStyle = activeObject.strokeLineCap;
            const cornerType = activeObject.cornerStyle;
            const fillColor = activeObject.cornerStyle;
            ...
            if(fillColor && fillColor !== null) {            
              $(`${_self.containerSelector} .toolpanel#select-panel .fill-section #color-picker`).val(fillColor).trigger("change")
            }
            if(borderColor && borderColor !== null) {
              $(`${_self.containerSelector} .toolpanel#select-panel .border-section #color-picker`).val(borderColor).trigger("change")
            }
            if(borderWidth && borderWidth !== null) {              
              $(`${_self.containerSelector} .toolpanel#select-panel .border-section #input-border-width`).val(borderWidth).trigger("change")
            }
            if(borderStyle && borderStyle !== null) {               
              ...
              $(`${_self.containerSelector} .toolpanel#select-panel .border-section #input-border-style`).val(value).trigger("change")
            }
            if(cornerType && cornerType !== null) {  
              console.log(cornerType)            
              $(`${_self.containerSelector} .toolpanel#select-panel .border-section #input-corner-type`).val(cornerType).trigger("change")
            }
          }
        }
    }
